...
parser.add_argument('-pm','--print_mode', default='time')
...
parser.add_argument('-pt','--print_time', default=60*60, type=int)
parser.add_argument('-st','--stop_time', default=48*60*60, type=int)
...
def generate_samples(generate_fn, base_name):
	...
	samples = numpy.zeros((NUM_SAMPLES_TO_GEN, LENGTH_GEN+3), dtype='int32')
	samples[:,:2] = Q_LEVELS//2

	last_hidden = numpy.zeros((N_GRUS, NUM_SAMPLES_TO_GEN, DIM), dtype='float32')
	reset = floatX(0)

	for i in range(LENGTH_GEN-1):
		temp, last_hidden = generate_fn(samples[:,max(0,i-3*NUM_PIXEL_CNN_LAYER):i+3], last_hidden, reset)
		samples[:,i+2] = temp[:,-1]
...
train_fn = theano.function([sequences, h0s, reset], [cost, last_hidden], updates = updates)

valid_fn = theano.function([sequences, h0s, reset], [cost, last_hidden])

generate_fn = theano.function([sequences, h0s, reset], [next_samples, last_hidden])
...
cost_dict = {'training':[], 'iters':[]}


for seqs, reset, epoch in train_data_feeder:
